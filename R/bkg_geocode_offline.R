...
#'
#' Exponent that controls how strongly individual score components influence the
#' overall geocoding score. The final score is computed as
#' \code{place_score^hierarchical_weight * street_score^hierarchical_weight * house_number_score^hierarchical_weight}.
#' Lower values are more lenient towards imperfect matches (e.g. minor spelling
#' variations or house number additions like "1b" matched to "1"), while higher
#' values penalize them more strongly. A value of 0 ignores all component scores
#' (score is always 1), a value of 1 multiplies them directly (the geometric
#' mean). Defaults to \code{0.5}.
...
#'
#' @returns Returns an \code{sf} dataframe of class \code{GeocodingResults} with
#' the same number of rows as the input data. Each row contains the geocoding
#' result with score columns (\code{score}, \code{place_score},
...
#' the \code{hierarchical_weight} parameter.
#'
...
bkg_geocode_offline <- function(
  ...
  if (engine == "duckdb") {
    ...
    matched_rows$place <- matched_rows$place_matched
    matched_rows$id <- matched_rows$.iid

    house_coordinates <- bkg_query_ga(
      unique(matched_rows$place_slug),
      data_from_server = data_from_server,
      data_path = data_path,
      credentials_path = credentials_path,
      verbose = verbose,
  ...
  if (nrow(unmatched_rows)) {
    # Create empty sf rows for unmatched addresses
    empty_sf <- cleaned_data[0, ]
    unmatched_sf <- unmatched_rows[, ".iid", drop = FALSE]

    # Add all columns from cleaned_data as NA
