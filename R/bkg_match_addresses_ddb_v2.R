#' Match addresses against BKG database (DuckDB version)
#'
...
#' @param data_path Path to the Parquet database.
#' @param data_from_server Whether to read from server.
...
bkg_match_addresses_ddb <- function(
  ...
  base_url <- if (data_from_server) "https://10.6.13.50:8544" else data_path
  ...
  relevant_slugs_sql <-
    paste0("'", relevant_slugs, "'") |> 
    paste0(collapse = ", ")
  
  # DBI::dbExecute(con, "
  #   CREATE SECRET noverify_addresses (
  #     TYPE HTTP,
  #     VERIFY_SSL 0
  #   )
  # ")
  ...
  #     paste(DBI::dbQuoteString(con, relevant_slugs), collapse = ",")
  #   )
  # )
  ...
  #   )
  # 
  # file.copy(from = parquet_paths, to = tempdir())
  
  # parquet_paths <- 
  #   file.path(base_url, sprintf("ga/place_slug=%s/*.parquet", relevant_slugs), fsep = "/" )
  # 
  # parquet_sql <- paste(sprintf("'%s'", parquet_paths), collapse = ", ")
  

  # 
  # parquet_sql <-
  #   paste0("'", parquet_paths, "'") |>
  #   paste(collapse = ", ")
  ...
  #   collapse = ","
  # )
  # SQL normalization: German Umlaut replacement + strip_accents + lower.
  
  # Mirrors dyn_comparator's tolower() — no non-alphanumeric stripping.
  # Used for house number comparison where reclin2 only lowercases.
  nk <- function(expr) {
    sprintf(
      paste0(
        "lower(strip_accents(",
  ...
  # Street normalization — mirrors the reclin2 path exactly:
  #   1. gsub("straße\\b", "X", street, ignore.case=TRUE)
  #   2. gsub("weg\\b",    "Y", street, ignore.case=TRUE)
  #   3. normalize_key() → stri_trans_general("de-ASCII") + tolower + gsub("[^a-z0-9]","")
  #
  # In SQL: (1) nk() handles umlauts+accents+lower, so "straße" is already
  
  # "strasse".  (2) Reduce strasse/weg at WORD BOUNDARIES (\b), not just end-of-
  # string ($), matching R's gsub("weg\\b",...).  (3) Strip all non-alphanumeric
  # chars to match normalize_key().  Use lowercase x/y because nk() already
  # lowered — they survive the [^a-z0-9] strip.
  ...
  sql <- sprintf("
    ...
    hn_match AS (
      SELECT
        ...
        addr2.house_number AS hn_out,
        addr2.house_number_add AS hn_add_out,
  ...
  ",
                 nk_street("street"),
                 nk("house_number || COALESCE(house_number_add, '')"),
                 parquet_sql,
                 nk_street("street_clean"),
                 nk("COALESCE(hn_input, '')"),
                 input_tbl,
                 zip_code, place
  )
  ...
  # Penalty for non-matching house numbers
  geocoded$house_number_score[
    geocoded$hn_input != paste0(geocoded$hn_out, geocoded$hn_add_out)
  ] <- geocoded$house_number_score[
    geocoded$hn_input != paste0(geocoded$hn_out, geocoded$hn_add_out)
  ] - house_number_penalty
  ...
  if (house_number != "") {
    ...
    result[[paste0(house_number, ".y")]] <- paste0(geocoded$hn_out, geocoded$hn_add_out)
  }
