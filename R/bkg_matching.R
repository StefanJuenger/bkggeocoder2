...
bkg_match_addresses_ddb <- function(
  ...
  # Used for house number comparison.
  nk <- function(expr) {
    sprintf(
      paste0(
        "lower(strip_accents(",
  ...
  #   3. Strip all non-alphanumeric chars (mirrors normalize_key())
  ...
  mismatch <- geocoded$hn_input != paste0(geocoded$hn_out, geocoded$hn_add_out)
