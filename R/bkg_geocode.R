...
#'
#' @return \code{bkg_geocode} returns an \code{sf} dataframe of class
#' \code{GeocodingResults} containing the geocoding results. Each row
#' corresponds to an input address (or query) with score columns and matched
#' address components. Rows where geocoding failed have \code{NA} scores.
...
bkg_geocode <- function(.data = NULL,
  ...
  structure(
    ...
    type = "bkg",
    args = args,
    class = c("GeocodingResults", class(geocoded_data))
  )
}
...
check_online_args <- function(.data = NULL,
  ...
  if (!structured && is.null(query)) {
    cli::cli_abort("Either {.var .data} or {.var query} must be provided.")
  }

  if (!is.character(cols) && !is.numeric(cols)) {
    cli::cli_abort("The {.var cols} argument must contain either column names or column indices.")
  }
  ...
  if (!is.character(epsg) && !is.numeric(epsg)) {
    cli::cli_abort("The {.var epsg} argument must contain a valid EPSG code.")
  }
}